...
const RadicalBtn = ({
    ...
    const cn1 = isDisabled
        ...
        : isTouchDevice
        ? ""
        : "hover:bg-[#6495ed] hover:text-black hover:border-4 hover:border-solid hover:bg-[#6495ed] rounded-sm";
    ...
    return (
        <>
            {Object.keys(radicalsGroupedByStrokeCount).map((stroke) => {
                const keyValue =
                    stroke as keyof typeof radicalsGroupedByStrokeCount;
                return (
                    <React.Fragment key={stroke}>
                        {radicalsGroupedByStrokeCount[keyValue].map(
                            (radical, index) => {
                                const isSelected = value.has(radical);
                                const isDisabled = possibleRadicals == null
                                    ? false
                                    : (possibleRadicals as Set<string>).has(
                                            radical,
                                        ) || isSelected
                                    ? false
                                    : true;
                                    ...
                                    <React.Fragment key={radical}>
                                        ...
                                        <RadicalBtn
                                            ...
                                            onClick={(e) => {
                                                ...
                                                const prevHasRadical =
                                                    newSelected.has(radical);
                                                prevHasRadical
                                                    ? newSelected.delete(
                                                        radical,
                                                    )
                    ...
                    <ClearFiltersCTA
                        defaultMsg={
                            <div className="block-inline">
                                Try:
                                {externalLinks.slice(0, 5).map(
                                    (item, index) => {
                                        return (
                                            <span
                                                className="block-inline my-1"
                                                key={item.name}
                                            >
                                                <ExternalTextLink
                                                    href={item.url("捜")}
